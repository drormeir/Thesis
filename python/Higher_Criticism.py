...
class Extreme_P_Values_Discovery_Method:
    ...
    def __init__(self, name: str, gamma: str | float | list[float|str] | np.ndarray,\
                 selection_method: str,\
                 save_objectives: bool,\
                 str_param: str = '') -> None:
        if isinstance(gamma,str):
            assert gamma in ['logsqrt', 'all']
            if gamma == 'logsqrt':
                self.gamma = [gamma]
            else:
                self.gamma = gamma
        ...
        if isinstance(self.gamma,list):
            for g in self.gamma:
                if isinstance(g,float):
                    ...
                    assert g == 'logsqrt'
            self.effective_gamma = np.zeros(shape=(len(self.gamma),), dtype=np.float32)
            self.str_gamma_list = []
            for g in self.gamma:
                if isinstance(g,str):
            ...
            self.str_gamma_list = []
        assert selection_method in Extreme_P_Values_Discovery_Method.Selection_Methods
        self.len_p_values_per_ind_gamma = np.empty(shape=0, dtype=np.int32)
        self.name = name
        self.str_param = str_param
        self.selection_method = selection_method
        self.num_discoveries = np.empty(shape=0, dtype=np.int32)
        self.best_objective = np.empty(shape=0, dtype=np.float32)
        self.i_N = np.empty(shape=0, dtype=np.float32)
        self.full_name_param = name # + f'({self.selection_method})'
        if self.str_param:
            self.full_name_param += ' ' + self.str_param
        self.full_name = self.full_name_param
        self.is_method_min = self.selection_method != Extreme_P_Values_Discovery_Method.Selection_Method_ArgMax
        self.is_method_max = ~self.is_method_min
        self.objectives = np.empty(shape=0) if save_objectives else None
        self.max_len_p_values = 0
    
    def get_num_gamma(self) -> int:
        if isinstance(self.effective_gamma, np.ndarray):
            return self.effective_gamma.size
        return -1

    def get_full_name_with_gamma(self, ind_gamma: int) -> str:
        assert self.str_gamma_list
    ...
    def calc_i_N(self, N: int):
        ...
        self.i_N = one_to_N.reshape(1,-1) / N
        if self.gamma == 'all':
            self.len_p_values_per_ind_gamma = one_to_N
        else:
            len_p_values_per_ind_gamma = np.empty_like(self.effective_gamma)
            for ind_gamma, gamma in enumerate(self.gamma):
                if isinstance(gamma,str):
                    len_p_values_per_ind_gamma[ind_gamma] = min(math.log(N)*math.sqrt(N),N)
                elif gamma <= 0:
                    len_p_values_per_ind_gamma[ind_gamma] = N**-gamma
                else:
                    len_p_values_per_ind_gamma[ind_gamma] = N*gamma
            self.len_p_values_per_ind_gamma = np.maximum(1,len_p_values_per_ind_gamma+0.5).astype(self.len_p_values_per_ind_gamma.dtype)
        self.effective_gamma = np.minimum((self.len_p_values_per_ind_gamma+1e-3)/N,1.0)
        self.gamma_ip = self.len_p_values_per_ind_gamma - 1
        self.max_len_p_values = self.len_p_values_per_ind_gamma.max() # it is not neccessary monotonous
        self.max_effective_gamma = self.effective_gamma.max()
        self.argsort_N_gamma = self.len_p_values_per_ind_gamma.argsort()
    ...
    def run_sorted_p(self, p_values_sorted: np.ndarray) -> None:
        ...
        max_N = min(self.max_len_p_values, objectives.shape[1])
        num_gamma = min(self.len_p_values_per_ind_gamma.size, max_N)
        argsort_N_gamma = self.argsort_N_gamma[:num_gamma]
        self.best_objective = np.empty(shape=(num_p_vectors,num_gamma), dtype=np.float32)
        objectives = objectives[:,:max_N]
        if self.selection_method == Extreme_P_Values_Discovery_Method.Selection_Method_LastNegative:
            selected_indexes = np.empty_like(self.best_objective, dtype=np.int32)
            below_threshold_reversed = (objectives <= 0)[:,::-1]
            first_ind_gamma = argsort_N_gamma[0]
            first_N_gamma = self.len_p_values_per_ind_gamma[first_ind_gamma]
            ...
            for ind_ind_gamma, curr_ind_gamma in enumerate(argsort_N_gamma[1:], start=1):
                prev_ind_gamma = argsort_N_gamma[ind_ind_gamma-1]
                prev_N_gamma = self.len_p_values_per_ind_gamma[prev_ind_gamma]
                curr_N_gamma = self.len_p_values_per_ind_gamma[curr_ind_gamma]
        ...
        elif self.selection_method == Extreme_P_Values_Discovery_Method.Selection_Method_ArgMin:
            selected_indexes = np.empty_like(self.best_objective, dtype=np.int32)
            first_ind_gamma = argsort_N_gamma[0]
            first_N_gamma = self.len_p_values_per_ind_gamma[first_ind_gamma]
        ...
        elif self.selection_method == Extreme_P_Values_Discovery_Method.Selection_Method_ArgMax:
            selected_indexes = np.empty_like(self.best_objective, dtype=np.int32)
            first_ind_gamma = argsort_N_gamma[0]
            first_N_gamma = self.len_p_values_per_ind_gamma[first_ind_gamma]
            ...
            selected_indexes = np.repeat(self.len_p_values_per_ind_gamma.reshape(1,-1), repeats=num_p_vectors, axis=0)
            ...
            for ind_objective in objectives.argsort(axis=1).T:
                ind_objective = np.repeat(ind_objective.reshape(-1,1), repeats=self.len_p_values_per_ind_gamma, axis=1)
        ...
        self.num_discoveries = selected_indexes + 1
        '''
        invalid_selections = self.best_objective > 0
        self.num_discoveries[invalid_selections] = 0
        '''
...
class Bonferroni(Extreme_P_Values_Discovery_Method):
    def __init__(self,\
                 alpha: float = 1,\
                 gamma: str | float | list[float|str] | np.ndarray = [1.],\
                 selection_method: str = Extreme_P_Values_Discovery_Method.Selection_Method_LastNegative,\
...
class Benjamini_Hochberg(Extreme_P_Values_Discovery_Method):
...
class Lowest_Angle(Extreme_P_Values_Discovery_Method):
    def __init__(self, gamma: str | float | list[float|str] | np.ndarray = [1.],\
                 selection_method: str = Extreme_P_Values_Discovery_Method.Selection_Method_ArgMin,\
                 save_objectives: bool = False):
...
class Kolmogorov_Smirnov(Extreme_P_Values_Discovery_Method):  # Kolmogorov-Smirnov test
    def __init__(self, mode: int = 1, gamma: str | float | list[float|str] | np.ndarray = [1.],\
                 selection_method: str = Extreme_P_Values_Discovery_Method.Selection_Method_ArgMin,\
...
class Import_HC(Extreme_P_Values_Discovery_Method):
    def __init__(self, gamma: list[float|str] | np.ndarray = [0.3], stable: bool = True,\
        ...
        super().__init__(name='Import HC ' + ('Stable' if stable else 'Unstable'),
                         gamma=gamma, selection_method=Extreme_P_Values_Discovery_Method.Selection_Method_ArgMax, save_objectives=save_objectives)
        self.stable = stable
...
class Higher_Criticism(Extreme_P_Values_Discovery_Method):
    def __init__(self, gamma: str| float | list[float|str] | np.ndarray, selection_method: str = Extreme_P_Values_Discovery_Method.Selection_Method_ArgMax,\
    ...
    def plot_objectives(self, title: str = 'Higher Criticism objectives values'):
        ...
        num_discoveries = self.num_discoveries[0]
        ...
        plt.vlines(x=num_discoveries, ymin=hc_objectives.min(), ymax=hc_objectives.max(),
                   label=f'Optimal HC discovers {num_discoveries} best objective value={best_objective:.2f}',
...
class Berk_Jones(Extreme_P_Values_Discovery_Method):
    def __init__(self, selection_method: str = Extreme_P_Values_Discovery_Method.Selection_Method_ArgMin,\
                 gamma: str | float |list[float|str] | np.ndarray = [1.0],\
                 save_objectives: bool = False) -> None:
