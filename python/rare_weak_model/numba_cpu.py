...
if not globals.cpu_njit_num_threads:
    ...
    import math
    import numba
    from python.random_integers import numba_cpu as random_integers
    ...
    def modify_p_values(out: np.ndarray, mu: np.float64) -> None:
        ...
        for ind in numba.prange(rows*cols):
            row = ind // cols
            col = ind % cols
            isf = standard_normal_isf_newton(out[row][col])
            out[row][col] = standard_normal_sf(isf + mu)
    ...
    def random_p_values_matrix(num_steps: np.uint32, offset_row0: np.uint32, offset_col0: np.uint32, out: np.ndarray) -> None:
        ...
        out[:] = (work_array.astype(np.float64) + np.float64(0.5)) / math.ldexp(1.0, 64)
